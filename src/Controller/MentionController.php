...
class MentionController extends AbstractController
{
    ...
    public function create(
        ...
        // Détection des mentions @email dans le contenu
        // Regex qui trouve tous les @email dans le texte
        ...
        foreach ($mentionedEmails as $email) {
            // On vérifie que l'utilisateur mentionné existe
            $mentionedUser = $em->getRepository(User::class)->findOneBy(['email' => $email]);

            if (!$mentionedUser) continue;

            // On ne se mentionne pas soi-même
            if ($email === $user->getEmail()) continue;

            // On crée la mention
            $mention = new Mention();
            $mention->setMentionedEmail($email);
